...
const MultiRangeSlider = React.forwardRef((props, ref) => {
	...
	return (
		<div className={baseClassName} onWheel={onMouseWheel} ref={ref}>
			...
			{label && (
				<div className='label'>
					<div className='label-min'>{props.minValue%2==0? props.minValue/2+"h00": Math.floor(props.minValue/2)+"h30"}</div>
					<div className='label-min'>{props.maxValue%2==0? props.maxValue/2+"h00": Math.floor(props.maxValue/2)+"h30"}</div>
				</div>
			)}
		</div>
	);
